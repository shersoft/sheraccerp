  ...
  selectWidget() {
    return nextWidget == 0
        ...
        : nextWidget == 1
            ...
            : nextWidget == 2
                ? selectProduct() //selectProductWidget()
                : nextWidget == 3
                    ? itemDetailWidget()
                    : nextWidget == 4
                        ? cartProduct()
    ...
    Navigator.push(
        ...
        MaterialPageRoute(
            builder: (_) => SelectProduct(
                  ...
                  cessOnNetAmount: cessOnNetAmount,
                  enableKeralaFloodCess: enableKeralaFloodCess,
                  isMinimumRatedLock: isMinimumRatedLock,
                  negativeStock: negativeStock,
                  pRateBasedProfitInSales: pRateBasedProfitInSales,
                  salesmanAsVehicle: salesmanAsVehicle,
                  useOldBarcode: useOldBarcode,
                  useUniqueCodeAsBarcode: useUniqueCodeAsBarcode,
                  vehicleData: vehicleData,
                  itemStockAll: itemStockAll,
                  isStockProductOnlyInSalesQO: isStockProductOnlyInSalesQO,
                  formattedDate: formattedDate,
                  enableBarcode: enableBarcode,
                  itemCodeVise: itemCodeVise,
                  decimal: decimal,
                  isTax: isTax,
                  isLedgerWiseLastSRate: isLedgerWiseLastSRate,
                  rateTypeItem: rateTypeItem,
                  isSerialNoInStockVariant: isSerialNoInStockVariant,
...
class SelectProduct extends StatefulWidget {
  ...
  SelectProduct({
    Key key,
    @required this.enableMULTIUNIT,
    @required this.pRateBasedProfitInSales,
    @required this.negativeStock,
    @required this.cessOnNetAmount,
    @required this.enableKeralaFloodCess,
    @required this.useUniqueCodeAsBarcode,
    @required this.useOldBarcode,
    @required this.isMinimumRatedLock,
    @required this.salesmanAsVehicle,
    @required this.vehicleData,
    @required this.itemStockAll,
    @required this.isStockProductOnlyInSalesQO,
    @required this.formattedDate,
    @required this.enableBarcode,
    @required this.itemCodeVise,
    @required this.decimal,
    @required this.isTax,
    @required this.isLedgerWiseLastSRate,
    @required this.rateTypeItem,
    @required this.isSerialNoInStockVariant,
    @required this.isFreeItem,
    @required this.isEnableProfitlessSalesWarning,
    @required this.ledgerModel,
  }) : super(key: key);
...
class _SelectProductState extends State<SelectProduct> {
  bool isItemData = false, isVariantSelected = false, outOfStock = false;
  String itemLike = 'a';
  DioService api = DioService();
  ...
  Widget build(BuildContext context) {
    ...
    deviceSize = MediaQuery.of(context).size;
    return selectProductWidget();
  }

  selectProductWidget() {
    if (widget.salesmanAsVehicle) {
      double squareFeet = widget.vehicleData != null
          ? widget.vehicleData['Salary'].toDouble()
          : 0;
      if (squareFeet > 0) {
        _quantityController.text = squareFeet.toString();
      }
    }
    if (!widget.itemStockAll) {
      return FutureBuilder<List<StockItem>>(
        future:
            (salesTypeData.type == 'SALES-O' || salesTypeData.type == 'SALES-Q')
                ? widget.isStockProductOnlyInSalesQO
                    ? api.fetchStockProductLike(
                        DateUtil.dateDMY2YMD(widget.formattedDate), itemLike)
                    : api.fetchNoStockProductLike(
                        DateUtil.dateDMY2YMD(widget.formattedDate), itemLike)
                : api.fetchStockProductLike(
                    DateUtil.dateDMY2YMD(widget.formattedDate), itemLike),
                  ...
                  return index == 0
                      ...
                      : InkWell(
                          child: Card(
                            child: ListTile(
                              title: Text(
                                  'Name : ${widget.itemCodeVise ? itemDisplay[index - 1].code.toString() + ' ' + itemDisplay[index - 1].name : itemDisplay[index - 1].name}'),
                              subtitle: Row(
                                mainAxisAlignment:
                                    MainAxisAlignment.spaceBetween,
                                children: [
        ...
        future: (salesTypeData.type == 'SALES-O' ||
            ...
            ? widget.isStockProductOnlyInSalesQO
                ? api.fetchStockProduct(
                    DateUtil.dateDMY2YMD(widget.formattedDate))
                : api.fetchNoStockProduct(
                    DateUtil.dateDMY2YMD(widget.formattedDate))
            : api.fetchStockProduct(DateUtil.dateDMY2YMD(widget.formattedDate)),
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            if (snapshot.data.isNotEmpty) {
              var data = snapshot.data;
                      ...
                      ? Padding(
                          ...
                          child: TextField(
                            ...
                            onChanged: (text) {
                              ...
                              setState(() {
                                itemDisplay = items.where((item) {
                                  var itemName = widget.itemCodeVise
                                      ? item.code.toString().toLowerCase() +
                                          ' ' +
                                          item.name.toLowerCase()
                                      : item.name.toLowerCase();
  ...
  calculate(StockProduct product) {
    if (widget.enableMULTIUNIT) {
      if (saleRate > 0) {
        if (_conversion > 0) {
          ...
          if (_focusNodeRate.hasFocus) {
            ...
            _rateController.text = rate.toStringAsFixed(widget.decimal);
          }
          //rate = r;
      ...
      if (_focusNodeRate.hasFocus) {
        ...
        _rateController.text = saleRate.toStringAsFixed(widget.decimal);
        rate = saleRate;
      } else {
        rate = _rateController.text.isNotEmpty
            ? double.tryParse(_rateController.text)
        ...
        ? CommonService.getRound(
            ...
            ((discount * 100) /
                (widget.cessOnNetAmount
                    ...
                    : (taxP + 100 + kfcP))))
        : discount;
    gross = CommonService.getRound(widget.decimal, ((rRate * quantity)));
    subTotal = CommonService.getRound(widget.decimal, (gross - rDisc));
    if (taxP > 0) {
    ...
    if (widget.enableMULTIUNIT && _conversion > 0) {
      profitPer = widget.pRateBasedProfitInSales
          ? CommonService.getRound(
              2, (total - (product.buyingPrice * _conversion * quantity)))
          : CommonService.getRound(widget.decimal,
  ...
  calculateText(StockProduct product) {
    ...
    if (widget.enableMULTIUNIT && rate > 0 && _conversion > 0) {
  ...
  showAddMore(BuildContext context, StockProduct product) {
    ...
    if (editItem) {
      pRate = product.buyingPrice;
      rPRate = product.buyingPriceReal;
      widget.isTax = taxable;
      taxP = widget.isTax ? product.tax : 0;
      cess = widget.isTax ? product.cess : 0;
      cessPer = widget.isTax ? product.cessPer : 0;
      adCessPer = widget.isTax ? product.adCessPer : 0;
      kfcP = widget.isTax
          ? widget.enableKeralaFloodCess
              ? kfcPer
              : 0
          : 0;
      saleRate = _rateController.text.isNotEmpty
      ...
      cartModel.uniqueCode = uniqueCode;
      // cartModel.stock = product.quantity;
      if (widget.isLedgerWiseLastSRate && lastRateStatus) {
        lastRateOfLedger(product);
      }
      calculate(product);
    } else {
      ...
      if (lastRateSelected) {
        ...
        if (rateEdited) {
          ...
          if (widget.rateTypeItem == null) {
            ...
            if (widget.rateTypeItem.name.toUpperCase() == 'RETAIL') {
              saleRate = product.retailPrice;
            } else if (widget.rateTypeItem.name.toUpperCase() == 'WHOLESALE') {
              saleRate = product.wholeSalePrice;
            } else if (widget.rateTypeItem.name.toUpperCase() == 'BRANCH') {
              saleRate = product.branch;
            } else {
              saleRate = product.sellingPrice;
            }
      ...
      if (saleRate > 0 &&
          ...
          _rateController.text.isEmpty) {
        _rateController.text = _conversion > 0
            ? (saleRate * _conversion).toStringAsFixed(widget.decimal)
            : saleRate.toStringAsFixed(widget.decimal);
        rate = _conversion > 0 ? saleRate * _conversion : saleRate;
      }
      uniqueCode = product.productId;
      if (widget.isSerialNoInStockVariant) {
        _serialNoController.text = product.serialNo;
      }
      if (widget.isLedgerWiseLastSRate && lastRateStatus) {
        lastRateOfLedger(product);
                      ...
                      children: [
                        ...
                        MaterialButton(
                          ...
                          onPressed: () {
                            ...
                            setState(() {
                              isVariantSelected = false;
                              if (quantity > 0 || widget.isFreeItem) {
                                if (outOfStock) {
                                  ScaffoldMessenger.of(context)
                                      .showSnackBar(SnackBar(
                                    content: const Text(
                                  ...
                                  if (widget.isMinimumRatedLock) {
                                    ...
                                    bool profitable = true;
                                    if (widget.isEnableProfitlessSalesWarning) {
                                      if (profitPer > 0) {
                                        profitable = true;
                                      } else {
                                        profitable = false;
  ...
  productTrackingList(StockProduct product) {
    var ledId = widget.ledgerModel.id.toString().isNotEmpty
        ? widget.ledgerModel.id.toString()
        : '0';
    return showDialog(
        context: context,
        builder: (BuildContext context) {
    ...
    api.getProductTracking(product.itemId, ledId).then((value) {
      ...
      Map item = data.firstWhere(
          (element) =>
              ...
              widget.ledgerModel.name.toString().toLowerCase(),
          orElse: () => {});
